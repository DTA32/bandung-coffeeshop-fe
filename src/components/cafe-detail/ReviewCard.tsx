...
export default function ReviewCard({ content, visited_at }: ReviewCardProps) {
  ...
  const visited_at_formatted = visited_at ? new Date(visited_at).toLocaleString('en-ID', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  }) : null
      ...
      <div className="flex flex-col gap-3 text-moss-dark antialiased">
        {paragraphs.length > 0
          ...
          : <div className="flex justify-center items-center h-24">
              <p className="text-sm leading-[1.7] m-0 text-bark"> No review about this cafe yet. </p>
            </div> 
        }
      </div>
    </div>
